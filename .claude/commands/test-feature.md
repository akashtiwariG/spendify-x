description: Writes and runs tests for a specific
Spendly feature. Pass the spec name as argument e.
g.
...
<spec-name> e.g. /test-feature
05-backend-connection"

If `.claude/specs/$ARGUMENTS.md` does not exist,
stop
immediately and say:
"Spec file not found at .claude/specs/$ARGUMENTS.
md.
...
says
  the feature SHOULD do. Do NOT derive test logic
  from
  reading the implementation. Cover happy paths,
  edge
  cases, auth guards, validation errors, and DB
  side
  effects.
...
**test-writer_agent** subagent with the following
context:
...
- Run command:
  ...
  by
  cross-referencing the test code, the spec, and
  the
...
- Do NOT run any tests beyond `tests/
test_$ARGUMENTS.py`
- If spendly-test-writer reports it could not write
  the test file, stop and report the reason - do
  NOT
  proceed to Step 2
...
- List each test written with a one-line
description
